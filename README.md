...
- **front/cep-front/**: Aplicação React para busca de CEP e gerenciamento de notícias (CRUD), com autenticação simulada.
- **front/back-noticias/**: API fake de notícias usando json-server.

**Como rodar:**
1. Instale dependências em cada pasta:
   - `cd front/cep-front && npm install`
   - `cd front/back-noticias && npm install`
2. Inicie o backend:
   - `cd front/back-noticias && npx json-server --watch db.json --port 3001`
3. Inicie o frontend:
   - `cd front/cep-front && npm run dev`

Acesse: [http://localhost:5173](http://localhost:5173)

**Estrutura:**
- `front/cep-front/`: React + Vite + TypeScript
- `front/back-noticias/`: json-server

Para acessar notícias, é necessário login. Projeto responsivo e pronto para Docker.
